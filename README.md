...
| 2 | 想要查看与该图片同一分类下的其他图片 | 通用物体和场景识别/图像标签分类 | 次重要 | ff |
...
| 2 | 通用物体和场景识别/图像标签分类 | 可能由于图片问题导致返回的分类结果有差错 | xx% | 分类的内容可能没有一定的关联 | 问题不大，用户体验可能会降低 |
| 3 | 图像搜索 | 上传的图片有一定的要求——尽量避免背景、其他干扰特征，否则在计算图片相似度时会有误差，造成检索结果不准确 | xx% | 输出的结果可能不是用户期望的 | 问题不大，用户体验可能会下降 |
| 4 | 图片内容审核 | 可能由于判断不准确导致某些合规的图片不能上传 | xx% | 用户不能上传某些“擦边”的图片 |问题不大，属于概率极小情况 |
...
## 3.3 API产品使用关键AI或机器学习之API的输出入展示 15%
### API1.使用水平 5%
